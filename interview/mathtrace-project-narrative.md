...
- AI 输出不能带 `invalid_ai_json`（AI JSON 非法）、`invalid_ai_schema`（AI schema 非法）、`unknown_tag_removed`（未知标签被移除）等结构性 invalid warning（非法输出警告）。
- `invalid_evidence_terms_removed`（有证据词被清洗移除）不再一票否决。只要清洗后每个 AI 标签仍至少保留一个有效 `evidence_terms`（证据词），它会作为 `ai_evidence_terms_partially_removed`（AI 部分证据词被移除）进入审计理由；如果某个 AI 标签清洗后没有任何证据，则进入 `missing_ai_evidence`（AI 标签缺少有效证据）并转人工审核。
...
- `method_tags`（解题方法标签）和非图像类 `feature_flags`（题目特征标记）允许 AI 补充，但必须是 taxonomy 已定义标签，并且每个 AI 标签都保留有效 `evidence_terms`（证据词）。
...
第五个取舍是：允许 AI 补充 metadata（元数据），但不允许它绕过 gate（门控）直接写最终事实。P2.3c 里，`tag-proposal-merge-core.mjs`（标签建议合并和门控核心文件）把 `method_tags`（解题方法标签）和非视觉 `feature_flags`（题目特征标记）从“必须和规则完全一致”改成“高置信、有有效证据、且标签存在于 taxonomy 时可以补充”。例如规则只识别出切线斜率题，AI 额外指出题干里有 `ln x`（自然对数表达）和参数 `a`，系统可以把 `logarithmic_derivative_formula`（对数求导公式）和 `has_parameter`（含参数）补进最终标签。这里放宽的是检索和推荐会用到的辅助 metadata，不是目标能力的事实边界；`target_skills`（目标能力标签）冲突、`needs_visual`（依赖图像）和缺少证据仍然会进入人工审核队列。
...
自动通过门控是保守设计。只有 AI 高置信度、输出合法、规则和 AI 目标能力不冲突、题目不依赖图像、且每个 AI 标签都有有效证据时，系统才会生成 `auto_tag_review_records.json`（自动通过的标签审核记录文件）。P2.3c 之后，AI 可以补充 rule 漏掉的 `method_tags`（解题方法标签）和非视觉 `feature_flags`（题目特征标记），但这些补充必须来自 taxonomy 允许的标签，不能现场发明。目标能力冲突、缺少目标能力、图像依赖、低置信度或证据缺失的题仍然进入 `tag_review_queue.json`（人工标签审核队列文件），再由本地 review UI（审核界面）导出 `tag_review_records.json`（人工标签审核记录文件）。最终 `final_tag_review_records.json`（最终标签审核记录文件）里，人工记录可以覆盖自动记录。

证据词校验这块我做过一次校准。AI 输出的 `evidence_terms`（证据词）必须能在题干、检索文本或规则证据里找到；找不到的会进入 `removed_evidence_terms`（被清洗移除的证据词列表）。但我没有把 `invalid_evidence_terms_removed`（有证据词被清洗移除）继续作为一票否决，因为真实教辅 OCR 和 LaTeX 表达会有格式差异。现在的策略是：部分证据词被删但每个标签仍有有效证据时，可以自动通过并保留审计理由；如果某个标签清洗后没有任何有效证据，才用 `missing_ai_evidence`（AI 标签缺少有效证据）转人工审核。这个设计让 gate（门控）不是拍脑袋放宽，而是把证据校验从“格式严格匹配”调整成“可追溯质量信号”。

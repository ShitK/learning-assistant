...
- AI 正常参与门控时，不能带 `invalid_ai_json`（AI JSON 非法）、`invalid_ai_schema`（AI schema 非法）、`unknown_tag_removed`（未知标签被移除）等结构性 invalid warning（非法输出警告）。但如果 AI 偶发返回坏 JSON，且 rule proposal（规则标签建议）已经给出明确目标能力，P2.3e 起可以走 `rule_only_fallback`（只用规则标签兜底）自动通过。
...
第三个取舍是：自动通过门控要保守，但不能让 AI 偶发失败变成单点阻塞。高置信度且规则与 AI 一致的题可以自动通过，降低人工审核量；如果 AI 返回坏 JSON 或空标签，但 rule proposal（规则标签建议）已经明确识别出目标能力，P2.3e 起可以走 `rule_only_fallback`（只用规则标签兜底）自动通过。真正需要进 review queue（人工审核队列）的，是目标能力完全冲突、规则和 AI 都缺目标能力、需要图像或 AI 输出无法由规则兜底的题。

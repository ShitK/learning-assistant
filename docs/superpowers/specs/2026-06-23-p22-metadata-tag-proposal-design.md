...
- 题干包含“切线”“斜率” -> `["切线斜率"]`
- 题干包含“极限”且包含导数定义式形态 -> `["极限式识别导数"]`
- 题干包含“单调”“递增”“递减” -> `["单调性"]`
- 题干包含“极值”“最值”“最大值”“最小值” -> `["极值最值"]`
- 题干包含“零点”“根”“交点” -> `["零点"]`
- 题干包含“参数”“恒成立”“取值范围” -> `["参数范围"]`
...
`target_skills` 偏“练什么能力”，`method_tags` 偏“用什么方法”。两者会有重叠，但不完全相同。
...
- 有 `sqrt`、`√`、`根号` -> `has_root`
- 来源或 warning 显示图像依赖 -> `needs_visual`
...
- `source` 第一版只允许 `"rule"`；未来可扩展 `"llm"` 或 `"human"`。
...
P2.2 第一版可以不做复杂审核 UI。如果 69 道题的规则命中质量足够高，可以先输出 proposal summary，让用户抽查 top warnings，再决定是否需要本地审核页。若要做审核页，应沿用 P2.0 本地静态工具边界，不做正式产品页面。
...
- 搜索打分增加：
  - query `target_skills` 命中 item `target_skills`
  - query 派生 `method_tags` 命中 item `method_tags`
  - feature flags 可作为 tie-breaker 或 warning
- `near_transfer` 判断从“不同章节 + target_skill 文本命中”升级为“不同章节 + 目标技能/方法标签命中”。
- `mixed_application` 判断从“不同章节但未命中 target_skill”升级为“不同章节 + 相关 method_tags 或 feature_flags，但不是同一 target_skill”。
...
- 如果 enriched corpus 仍不足，warning 语义比 P2.1 更具体。

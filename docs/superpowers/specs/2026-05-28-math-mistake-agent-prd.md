...
P1 `image_diagnosis` 后端通过 vision provider adapter 调用多模态接口。模型只负责图片 OCR 和结构化抽取，输出 `question_text`、`student_answer`、`student_solution_steps`、`extraction_confidence` 和 `warnings`；不要求、也不接收视觉模型生成标准解法。模型输出必须先经过 JSON 解析和边界校验，再返回 `extraction_review` 识别草稿；用户确认后才通过 `/api/confirm` 进入确定性 Pipeline。最终报告中的 `mistake_diagnosis.standard_solution` 由确认后的 text analysis provider 基于用户确认文本生成；provider 不可用时回退为本地保守提示。知识点映射、错因诊断、`memory_delta`、练习和复习计划仍由本地规则生成。

模型输出作为不可信外部输入处理。`student_solution_steps` 可以从字符串数组、多行字符串、或含 `text`/`content`/`step`/`value` 文本字段的对象数组规范化为内部字符串数组；空项和无法解释的项会被丢弃并生成 warning；超过上限的步骤会被截断。如果模型返回任何未声明字段，包括标准解法、画像、历史错题或掌握度变化等字段，解析层视为越权输出并返回可恢复的 `model_invalid_output`。非法 JSON、缺少题干、缺少学生作答区域、越权字段或未声明字段仍返回 recoverable `model_invalid_output`。数学表达式应尽量使用 `$...$` 或 `$$...$$` 包裹，便于前端用 KaTeX 稳定渲染；前端可做有限裸公式兜底，但不得把兜底正则当作完整数学解析器。
...
P1.5 的 DeepSeek/text analysis provider 边界是“展示增强”，不是“画像写入”。在 `problem_only` 路径中，只要用户确认后的题干证据不是 `insufficient`，后端可以可选调用 text analysis provider 补完整标准解法、题型解释、常见卡点说明或用户追问后的分析草稿；这些内容可以展示给学生，也可以作为用户审核前的草稿。确认令牌 fingerprint 不匹配时，报告仍可使用用户确认后的文本生成展示用标准解法，但必须强制 `memory_delta.should_persist=false`，不得写入长期画像。`evidence_level`、`persistence_evidence`、`profile_update_kind`、`memory_delta`、`student_profile` 和 localStorage 写入决策必须继续由本地证据规则决定。用户未回答追问时，分析模型不得把题目常见易错点直接升级为学生具体错因；用户回答追问后，分析草稿也必须经用户确认，确认后由本地规则把已确认卡点映射到受控错因标签。
...
P2.12 在既有 `/api/diagnose` 和 `/api/confirm` 服务端受控流程中补齐轻量多智能体职责编排 MVP，但不改变 API 契约、前端 UI、Supabase schema 或画像写入规则。已实现的职责模块包括：`VisionExtractionAgent` 只负责 `image_diagnosis` 的图片输入校验、vision provider 调用、抽取草稿解析和确认令牌生成；`MistakeDiagnosisAgent` 负责把 `sample_diagnosis` 或用户确认后的图片草稿交给既有确定性诊断 pipeline，生成标准解法、错因诊断、练习和复习计划；`LearningMemoryAgent` 负责在诊断结果产生后复用现有 persistence gate，按 `memory_delta.should_persist`、证据等级和去重规则尝试写入 `diagnosis_runs`、`mistake_book_items`、`memory_events` 并同步 `student_profiles`。P2.12 的 Agent 是职责边界封装，不是自主推理 runtime：不引入 LangGraph 或 AutoGen，不做 agent 自主决策、checkpoint、interrupt、多 provider routing、多题历史或聊天消息持久化，也不允许模型自由生成或覆盖长期学习状态。
...
P0 自动确认并在 `/api/diagnose` 内完成画像更新或返回更新后的画像。P1 图片路径提供学生确认或修改识别文本的交互；确认后才进入知识点映射、错因诊断和画像写入决策。P1.7 起，服务端在确认后的诊断成功响应后尝试写入 Supabase Postgres；P1.8 起，成功持久化或删除错题后会 best-effort 同步当前画像快照：
...
- `student_profiles`：保存从 gated `memory_events` 投影出的当前画像快照，用于 `/api/student-profile` 读取。
...
- 前端不直连数据库；`SUPABASE_SERVICE_ROLE_KEY` 只允许服务端读取，浏览器只能通过 Next.js API 访问错题本和云端画像。
...
### P1.7/P1.8 Supabase Data Foundation

P1.7 的数据库目标是先建立可审计的数据底座，而不是一次性做完整学习系统。P1.8 在此基础上补一个当前画像快照 read model，方便工作台从云端恢复最新结构化画像。当前五张表的职责如下：
...
student_profiles
  保存从 gated memory_events 投影得到的当前学生画像快照，包括
  profile、profile_version、event_count 和 last_memory_event_id。
```

`memory_events` 独立成表，是为了让后续 Agent、老师端或复习规划可以追溯“为什么这个学生画像变了”，而不是只能读取最新画像分数。`student_profiles` 不是新的事实来源，而是从 `memory_events` 重建出来的当前画像快照，用于降低工作台读取当前画像的成本；删除错题导致相关 `memory_events` 变化后，也需要重新同步投影画像。localStorage 继续作为 demo fallback，避免数据库未配置、读取失败或云端尚无画像时破坏演示。

数据库写入只发生在服务端确认后的路径：`sample_diagnosis` 自动确认，`confirmed_image_diagnosis` 需要 `/api/confirm` 和服务端证据策略允许。`image_diagnosis` 的识别草稿、完整图片 base64 和未确认内容不进入错题本持久化，也不会直接写入 `student_profiles`。模型输出不得直接覆盖画像；当前画像只能由代码从受控 `memory_delta` 合并得到。
...
P1.8 的云端 `student_profiles` 表只保存当前聚合后的 `StudentProfile` JSON 快照和投影元数据，当前约束为 `student_id='demo_student_001'`、`subject='math'`，并且只授予 service role 读写权限。浏览器端读取路径固定为 `GET /api/student-profile?student_id=demo_student_001`，返回云端画像或 `profile=null` fallback；前端不能直接读取 Supabase。

P1.9 后，`student_profiles.profile.mastery_scores` 仍是云端画像快照的一部分。前端展示时可以派生“薄弱指数”，但不得把该派生值写回数据库，也不得让前端直接读取 Supabase 或完整 `memory_events`。P1.10 起，推荐依据可以通过 profile evidence API 使用服务端摘要后的最近画像事件证据；该接口仍不暴露完整事件、完整诊断运行、题目正文、学生答案或标准答案。
...
视觉抽取内部类型不再保留标准解法过渡字段。确认后由 `ANALYSIS_PROVIDER_*` 基于用户确认文本生成最终 `mistake_diagnosis.standard_solution`；provider 不可用时使用本地保守提示。
...
- Storage: 本地 JSON 文件 + 浏览器 localStorage fallback；P1.7/P1.8 使用 Supabase Postgres 保存诊断审计、错题本、画像事件和当前画像快照
...
输出：当前云端 student_profile；未配置、未生成或读取失败时返回 profile=null fallback
```
...
| `task_type=confirmed_image_diagnosis` 经 `/api/confirm` 提交 | 校验确认令牌、草稿和 `confirmation_action`，再进入确定性 Pipeline；只要确认后的题干证据不是 `insufficient`，即可可选调用 text analysis provider 增强报告文本，`problem_only` 也可用它补完整标准解法和追问分析草稿；fingerprint 不匹配时只禁止画像写入 | 可选调用 `ANALYSIS_PROVIDER_*`；provider 失败、越权输出或证据不足时回退本地规则 | 使用知识库映射、证据评估、错因规则、memory_delta 规则和现有练习/复习模板；分析模型只覆盖展示文本，不决定画像写入 | 200，`fallback_used=false` |
...
图片识别预览响应只携带题干、学生答案、学生步骤和识别置信度；前端确认区不展示、也不编辑标准解法。最终报告中的 `standard_solution` 优先由确认后的 text analysis provider 生成，provider 不可用时回退为本地保守提示。

`confirmation_token` 只作为不透明字符串交给前端；token payload 不包含完整题干、学生答案、标准解法或图片内容。生产环境必须配置 `MATHTRACE_CONFIRM_SECRET` 用于签名和草稿指纹校验；开发环境优先使用 `MATHTRACE_CONFIRM_SECRET`，未配置时回退到稳定本地 demo secret，不得回退到 `VISION_PROVIDER_API_KEY` 或 legacy `MIMO_API_KEY`，避免切换模型配置时让未确认草稿失效。确认后完整图片诊断响应仍使用 `source="image"`，并由服务端返回的 `memory_delta.should_persist` 决定是否允许前端写入 localStorage；前端不得再用低置信度自行否决服务端允许的 `problem_type_focus` 写入。
...
- 如果模型已返回题干和学生答案，但缺少 `extraction_confidence` 或 `warnings`，解析层可做有界补齐：置信度按 `low` 处理，warnings 缺失按空数组处理并追加安全提醒。此类结果仍必须进入用户确认，不得直接写入长期画像。
...
`ANALYSIS_PROVIDER_*` 与 `VISION_PROVIDER_*` 分离：前者只接收用户确认后的题干、学生答案、学生步骤和追问回答（如有），不接收图片 base64，也不依赖视觉模型生成标准解法；后者只负责图片抽取。未配置、请求超时、HTTP 失败、非法 JSON 或输出越权字段时，`/api/confirm` 必须回退到本地确定性规则报告，不阻塞确认流程，也不影响 `sample_diagnosis`。
...
确认令牌 fingerprint 不匹配时，可以调用 text analysis provider 生成展示用标准解法和报告文案，但不得写入长期画像。
...
- 内置知识库：项目 JSON 文件，只读。
- demo 学生画像：项目内置 `demoStudentProfile` 空历史基线；前端读取不到 localStorage 或解析失败时回退到该对象。
- 本次诊断结果：前端继续用 localStorage 恢复 demo 画像；P1.7 起，确认后的诊断运行、错题本条目和画像事件可写入 Supabase Postgres；P1.8 起，当前画像快照可通过 `/api/student-profile` 从 `student_profiles` 读取。
...
-> 如果不存在、不可解析或结构不匹配，则回退到空历史 demoStudentProfile
-> P1.8 best-effort 调用 /api/student-profile 拉取云端当前画像；失败则继续使用本地 fallback
...
刷新页面不会丢失 localStorage 中的 demo 状态；localStorage 损坏、缺失或读写失败时回退到空历史 `demoStudentProfile`。点击“重置画像”只清理浏览器 localStorage 和当前页面 session，不删除云端 `memory_events` 或 `student_profiles`；如果 Supabase 中已有 `demo_student_001` 的画像，页面后续仍可能通过 `/api/student-profile` 读回云端画像。P1.7/P1.8 的 Supabase 写入和画像读取不替代这套 demo fallback 模型，只补充可审计的 diagnosis run、mistake book item、memory event 和当前画像 read model。

`demoStudentContext.today_focus` 仍是工作台演示焦点文案，不是学生画像事实；当前画像事实只来自空历史 `demoStudentProfile` 加确认写入后的 `memory_delta` / `memory_events`。
...
Few-shot 输出示例见 Appendix B。Provider 失败时返回可恢复错误；模型请求失败或非法输出必须将 `fallback_used` 标记为 true，且不得写入画像。视觉 prompt 只要求模型输出题干、学生答案、学生步骤、置信度和 warnings。

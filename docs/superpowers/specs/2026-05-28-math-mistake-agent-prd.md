...
MVP 选择“错题诊断 + 长期画像 + 复习路线”版本，但黑客松实现必须收敛为稳定演示闭环。第一优先级是让评委完整看到：样例题诊断、错因报告、画像变化和变式练习。真实图片上传和 Kimi 识别作为强加分项实现，但必须有样例题降级路径。

### In Scope

- 高中数学单题诊断
- 内置 2-3 道高质量样例错题
- 尽量接入 Kimi 多模态模型识别图片；失败或未配置时提示用户切换到样例题演示
- 内置轻量高中数学知识库
- Agent 展示多步骤诊断过程
- 输出结构化错因报告
...
3. P0 正式演示固定走样例题路径；P1 图片路径才调用 Kimi 多模态模型识别题干、学生答案和解题步骤。
...
P0 中 `sample_diagnosis` 是正式演示路径，不是降级路径。`fallback_used=false`。只有当用户尝试 `image_diagnosis` 且 Kimi 不可用或返回异常时，才视为 fallback 或错误。
...
P0 中变式练习题来自样例题 JSON 中的预写题目，保证演示稳定。P1 再接入 Kimi 动态生成。练习题不应只围绕本题生成，还应参考学生历史画像。例如学生多次遗漏定义域时，变式题应刻意加入定义域或参数范围陷阱。
...
P0 自动确认并在 `/api/diagnose` 内完成画像更新或返回更新后的画像。P1 再提供学生确认或修改识别文本和诊断结果的交互。
...
MVP 阶段默认自动确认样例题诊断结果；真实图片上传场景可保留“编辑识别结果”和“确认写入”的交互空间作为 P1。
...
MVP 必须优先保证演示稳定。所有模型调用和知识库检索都要有降级路径。

| 场景 | 处理策略 | 用户看到什么 |
| --- | --- | --- |
| Kimi API 超时 | 15 秒后停止等待，使用样例题预标注结果 | 提示“模型响应较慢，已切换到演示样例” |
| Kimi 返回非法 JSON | 最多重试 1 次；仍失败则使用预标注诊断结果 | Agent 过程区显示“结构化输出校验失败，使用备用结果” |
| 图片识别置信度低 | 不直接写入长期记忆，要求学生编辑识别文本 | 识别结果进入可编辑状态 |
...
Demo 降级路径：如果真实图片上传、Kimi API 或 JSON 校验任一环节失败，系统提示用户改用样例题；样例题路径必须能用预标注数据走完“诊断 -> 画像变化 -> 变式练习 -> 复习建议”的完整流程。

正式演示路径不依赖 Kimi：演示者直接选择样例题，系统返回预标注的完整诊断 JSON。真实图片上传只作为 P1/演示后体验。
...
## 9. Kimi Integration Requirements

Kimi 用于多模态识别、结构化诊断和练习生成。
...
模型必须输出结构化 JSON，再由前端展示成报告。不要直接把自然语言长回答作为唯一结果。
...
```text
POST /api/confirm
输入：mistake_record、memory_delta、student_id
输出：更新后的 student_profile
```

`/api/confirm` 属于 P1。P0 中样例题诊断结果自动确认，持久化和画像更新在 `/api/diagnose` 中完成或由前端 localStorage 模拟完成。
...
| `task_type=image_diagnosis` 且 `KIMI_API_KEY` 已配置 | 执行图片识别与结构化诊断 | 调用 | 使用知识库检索和规则计算 | 200，`fallback_used=false` |
| `task_type=image_diagnosis` 且 `KIMI_API_KEY` 未配置 | 不做自动样例替换，返回可恢复错误 | 不调用 | 不执行 | 400，提示用户选择样例题 |
| Kimi 超时或非法 JSON | 返回可恢复错误，并提供样例题入口 | 可重试 1 次 | 不写入画像 | 502，`fallback_used=true` |
...
400 = 参数错误，例如未选择样例题、未上传图片、Kimi 未配置但请求 image_diagnosis
413 = 图片过大
500 = 服务端未知错误
502 = Kimi 超时或返回非法结构
```
...
- 前端应把图片压缩到 1MB 以内，避免 Next.js API Route body size 限制。
- 如果图片超过限制，前端提示用户压缩或重新拍照。
- 若后端需要支持更大图片，再调整 API Route body size 或改为 multipart/form-data；这属于 P1 实现细节。

### Kimi API Key Handling

Kimi API Key 只能放在服务端环境变量中，不允许前端直连。
...
- 本次诊断结果：保存在 localStorage，后续可迁移到 SQLite、Supabase 或云数据库。
...
| fallback | image_diagnosis 失败 | 展示错误提示和“改用样例题”按钮 |
...
## 19. Appendix A: Kimi Diagnosis Prompt Template

System prompt:

```text
你是“错因地图 MathTrace”的高中数学错题诊断 Agent。你的任务不是只给答案，而是基于题目、学生答案、知识库上下文和学生历史画像，输出结构化错因诊断。

必须遵守：
1. 优先使用给定知识点 ID 和错因标签，不要创造新的标签。
2. 如果信息不足，用 confidence 标记为 low，并说明需要学生确认。
3. 输出必须是合法 JSON，不要输出 Markdown。
4. 不要把一次低置信度诊断直接描述成确定事实。
```
...
Few-shot 输出示例见 Appendix B。Kimi 失败时返回可恢复错误；如实现 fallback 输出，必须与该 Schema 一致，并将 `fallback_used` 标记为 true。
...
当前没有阻塞实施计划的产品决策。实现阶段可配置 `.env.local` 中的 `KIMI_API_KEY`；若未配置，正式演示仍走 `sample_diagnosis` 样例题路径，`image_diagnosis` 返回可恢复错误并引导用户选择样例题。

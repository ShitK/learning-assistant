...
| 场景 | 处理策略 | 用户看到什么 |
| --- | --- | --- |
| Provider API 超时 | 15 秒后停止等待，返回 `model_timeout`，不写入画像 | 提示“模型响应较慢，请稍后重试或改用样例题” |
...
| `task_type=image_diagnosis` 且 `MIMO_API_KEY` 已配置 | 执行图片抽取，返回 `extraction_review` 草稿和 `confirmation_token` | 调用 MiMo | 不执行后续诊断，不写画像 | 200，`fallback_used=false` |
| `task_type=image_diagnosis` 且 `MIMO_API_KEY` 未配置 | 不做自动样例替换，返回可恢复错误 | 不调用 | 不执行 | 400，`model_not_configured` |
| Provider 超时、请求失败或非法 JSON | 返回可恢复错误，并提供样例题入口 | 调用失败 | 不写入画像 | 502，`fallback_used=true` |
| `task_type=confirmed_image_diagnosis` 经 `/api/confirm` 提交 | 校验确认令牌和草稿，再进入确定性 Pipeline | 不调用 | 使用知识库映射、错因规则、memory_delta 规则和现有练习/复习模板 | 200，`fallback_used=false` |
...
MiMo API Key 只能放在服务端环境变量中，不允许前端直连。
...
9. 接入 Anthropic-compatible 图片抽取 provider，当前 MiMo first。
10. 优化页面视觉和演示路径。
...
当前没有阻塞实施计划的产品决策。实现阶段可配置 `.env.local` 中的 `MIMO_API_KEY`；若未配置，正式演示仍走 `sample_diagnosis` 样例题路径，`image_diagnosis` 返回可恢复错误并引导用户选择样例题。

...
刷新页面不会丢失 localStorage 中的 demo 状态；localStorage 损坏、缺失或读写失败时回退到初始 mock 数据；点击“重置画像”可恢复初始 mock 数据。P1.7/P1.8 的 Supabase 写入和画像读取不替代这套 demo fallback 模型，只补充可审计的 diagnosis run、mistake book item、memory event 和当前画像 read model。

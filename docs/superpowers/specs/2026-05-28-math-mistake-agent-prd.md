...
P0 自动确认并在 `/api/diagnose` 内完成画像更新或返回更新后的画像。P1 图片路径提供学生确认或修改识别文本的交互；确认后才进入知识点映射、错因诊断和画像写入决策。
...
| `task_type=confirmed_image_diagnosis` 经 `/api/confirm` 提交 | 校验确认令牌和草稿，再进入确定性 Pipeline；fingerprint 匹配时可选调用 text analysis provider 增强报告文本 | 可选调用 `ANALYSIS_PROVIDER_*`；失败或 fingerprint 不匹配时回退本地规则 | 使用知识库映射、错因规则、memory_delta 规则和现有练习/复习模板；分析模型只覆盖展示文本 | 200，`fallback_used=false` |
...
`confirmation_token` 只作为不透明字符串交给前端；token payload 不包含完整题干、学生答案、标准解法草稿或图片内容。生产环境必须配置 `MATHTRACE_CONFIRM_SECRET` 用于签名和草稿指纹校验；开发环境可回退到本地 demo secret。确认后完整图片诊断响应仍使用 `source="image"`，并由 `memory_delta.should_persist` 和 `extraction_confidence` 共同决定是否允许前端写入 localStorage。
...
- 未确认、低置信度、确认令牌不匹配的图片识别结果只展示报告或识别草稿，不写入 localStorage 学生画像；模型仍不得直接写 `memory_delta` 或覆盖学生画像。
...
- 如果模型已返回题干和学生答案，但缺少 `standard_solution_draft`、`extraction_confidence` 或 `warnings`，解析层可做有界补齐：标准解法草稿使用“请根据题干补充标准解法”的待确认占位内容，置信度按 `low` 处理，warnings 缺失按空数组处理并追加安全提醒。此类结果仍必须进入用户确认，不得直接写入长期画像。
...
旧的 `MIMO_BASE_URL`、`MIMO_MODEL`、`MIMO_API_KEY` 仍作为本地兼容别名保留；新配置优先使用 `VISION_PROVIDER_*`。`provider_debug.provider_name` 默认使用 `anthropic_compatible_vision`，如需区分本地 provider，可显式设置 `VISION_PROVIDER_NAME`，但不得包含 API Key 或私密信息。OpenAI-compatible 图片字段默认使用 data URL；GLM-4.6V-FlashX 使用裸 base64，应配置 `VISION_PROVIDER_IMAGE_FORMAT=base64`。
...
`ANALYSIS_PROVIDER_*` 与 `VISION_PROVIDER_*` 分离：前者只接收用户确认后的题干、学生答案、学生步骤和标准解法草稿，不接收图片 base64；后者只负责图片抽取。未配置、请求超时、HTTP 失败、非法 JSON 或输出越权字段时，`/api/confirm` 必须回退到本地确定性规则报告，不阻塞确认流程，也不影响 `sample_diagnosis`。
...
```text
你是 MathTrace 的图片错题抽取器。你的任务不是诊断错因，而是从图片中抽取题目、学生答案、学生解题步骤和标准解法草稿。
...
Few-shot 输出示例见 Appendix B。Provider 失败时返回可恢复错误；模型请求失败或非法输出必须将 `fallback_used` 标记为 true，且不得写入画像。

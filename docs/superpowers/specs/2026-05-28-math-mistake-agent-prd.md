...
| `task_type=image_diagnosis` 且 `MIMO_API_KEY` 已配置 | 执行图片抽取，再进入确定性 Pipeline | 调用 MiMo | 使用知识库映射、错因规则、memory_delta 规则和现有练习/复习模板 | 200，`fallback_used=false` |
...
- 低置信度图片识别只展示结果，不写入 localStorage 学生画像；模型仍不得直接写 `memory_delta` 或覆盖学生画像。
...
- `image_diagnosis` 识别结果渲染、错误态和低置信度不持久化

...
P1.5 在 `/api/confirm` 后增加可信诊断降级路径。系统先评估证据等级：`student_work_sufficient` 表示识别到清晰学生答案和步骤，可进入具体错因诊断；`problem_only` 表示只识别到题干/标准解法或学生步骤不清，只能生成题型风险和快速追问；`insufficient` 表示题干或标准解法也不足，只提示重新上传或改用样例题。`problem_only` 不得直接写具体错因画像；用户跳过追问时只写 `profile_update_kind="problem_type_focus"`，对相关知识点固定轻微下调 `-2` 并加入 `review_priority_changes`，`mistake_cause_changes` 必须为空。用户选择/输入卡点后，系统先返回分析草稿给用户审核；只有用户再次确认后，才用 `persistence_evidence="user_confirmed"` 写入 `profile_update_kind="mistake_cause"`。
...
`ANALYSIS_PROVIDER_*` 与 `VISION_PROVIDER_*` 分离：前者只接收用户确认后的题干、学生答案、学生步骤、标准解法草稿和追问回答（如有），不接收图片 base64；后者只负责图片抽取。未配置、请求超时、HTTP 失败、非法 JSON 或输出越权字段时，`/api/confirm` 必须回退到本地确定性规则报告，不阻塞确认流程，也不影响 `sample_diagnosis`。

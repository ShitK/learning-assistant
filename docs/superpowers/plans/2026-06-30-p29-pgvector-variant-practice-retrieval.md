...
- [ ] **Step 6: Add route-level smoke test**

Create `scripts/tests/rag/variant-practice-route.test.mjs`:
...
If direct route import proves brittle under the current Jiti setup, move the same assertions into `scripts/tests/smoke/api-smoke.test.mjs` against the running app. Keep the invariant: HTTP-level serialization preserves the public API contract and hides internal pgvector fields.

- [ ] **Step 7: Add corpus source and route tests to default suite**
...
variant practice route tests passed
...
The CLI must not print API keys, service role keys, full embedding vectors, full provider responses, or full question text on error. File read/JSON parse failures exit before any repository write.
...
- [ ] **Step 1: Update README env guidance**

In `README.md`, add a local settings section near the existing provider/Supabase configuration:
...
- [ ] **Step 4: Update RAG artifact docs**
...
- [ ] **Step 5: Add interview narrative section**
...
按 Task 8 的真实验证结果填写。若代码、单元测试、smoke、lint 和 build 已完成，但真实 Supabase / embedding smoke 未运行，应写“代码与本地验证已完成，真实 Supabase / embedding smoke 待补充”。这个阶段把 P2.7 的本地 JSON 动态推荐升级为优先 pgvector 检索：审核通过的导数题库通过本地 CLI 同步到 Supabase Postgres + pgvector，运行时 `/api/variant-practice` 先用 query embedding 召回候选题，失败时回退本地 enriched corpus。
...
- P2.9 appears in README, PRD, roadmap, RAG artifact docs, interview narrative, and ADR.
...
- [ ] **Step 3: Real Supabase migration / embedding smoke**

This is a blocking verification when a real Supabase project and local `.env.local` values are available. Do not print env values, service role keys, embedding provider keys, full embedding vectors, or full provider responses.

Before applying the migration, inspect the existing vector extension location in Supabase SQL Editor:
...
- No row, or `schema_name = 'extensions'`.
- If `schema_name` is another schema such as `public`, stop and decide whether to recreate the project or write an explicit migration path; do not apply the P2.9 migration blindly.

Apply `supabase/migrations/20260630000000_p29_pgvector_variant_practice.sql` in Supabase SQL Editor, then verify:

```sql
select to_regclass('public.variant_practice_corpus_items') is not null as has_table;
select proname from pg_proc where proname = 'match_variant_practice_corpus_items';
```
...
- Removing one item from a temporary corpus copy and re-running apply deactivates the missing row; restoring it reactivates the row without forcing a new embedding when the hash is unchanged.

If a real Supabase project or env vars are not configured, record this as a residual verification gap in final notes:

```text
未运行真实 Supabase migration / embedding smoke：本地未配置相关 env 或真实 Supabase project；migration apply 仍需后续验证。
```

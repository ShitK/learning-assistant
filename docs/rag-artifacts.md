...
`artifacts/rag` 是本地生成目录，已被 `.gitignore` 忽略，不进入 Git。它保存 P2 RAG 工具链的 PDF/MinerU 解析、人工审核、题库生成、标签建议、标签审核、变式练习推荐产物，以及 P2.9 pgvector 同步源。
...
- `artifacts/rag` 只由本地脚本、服务端 loader/API fallback 和 demo artifact 工具读取；浏览器前端、Client Component 和产品页不直接读取本地文件或 artifacts。
- 产品页只接收服务端 API 或 Server Component 传入的 `ProductVariantPractice` 等产品 view-model 数据；`artifacts/rag/variant-practice-agent/recommendations.json` 缺失时，由服务端返回预写练习题 fallback。
- P2.7 动态变式练习 API 运行时在服务端读取 `artifacts/rag/enriched-practice-corpus/enriched_practice_corpus.json`，只消费已通过标签审核的题；产品页默认样例题的本地推荐结果也只通过服务端 loader 转成产品 view-model 后传给前端。
- P2.9 的 pgvector 同步源仍是本地 `artifacts/rag/enriched-practice-corpus/enriched_practice_corpus.json`。该 artifact 不提交 Git；`scripts/rag/sync-variant-practice-pgvector.mjs --apply` 只把审核通过、非视觉依赖的题同步到 Supabase `variant_practice_corpus_items`。运行时 pgvector 不可用时，服务端 `/api/variant-practice` 仍读取本地 enriched corpus fallback。
...
| `variant-practice-agent/recommendations.json` | 服务端 loader 读取并转成产品 view-model 的 3 道变式练习推荐 | 活跃推荐产物 | 可由 enriched corpus + demo query 再生成 | 保留 |
...
5. 清理后重新跑 P2.5 product view model smoke，确认服务端仍能向产品页提供 3 道推荐题 view-model。
